...
int main() {
	...
	inet_pton(AF_INET, "192.168.43.208", &client_info.ack.sin_addr);
	...
	inet_pton(AF_INET, "192.168.43.208", &server_info.data.sin_addr);
	...
	inet_pton(AF_INET, "192.168.43.208", &server_info.ack.sin_addr);
	...
	unsigned int idx[NUM_OF_CLIENTS] = {0};
		...
		if (FD_ISSET(server_sock.data, &rset)) {		// if UDP packet was received
			...
			if (!crc_check) {	//packet corrupted
				...
				if(curr_recv_seq_ID == 0 && !already_came){
					...
					memcpy(&file_data[idx[curr_client_ID] * DATA_LEN], Buffer_Rec, curr_recv_seq_ID_idx);
				}
				already_came = true;
			}
		}
